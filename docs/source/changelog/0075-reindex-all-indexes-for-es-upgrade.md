...
[045d45fa](https://github.com/kabass/commcare-hq-benin/commit/045d45fa69b6dd83f870ed185c0d73adeef14350)
...
3. Check disk usage on each node
    ...
    If you don't have enough space available in your elasticsearch cluster, it is recommended to add more storage capacity before reindexing. If you don't have the option to add more storage, you will need to reindex one index at a time. You should follow the process described in [Reindexing One Index at A Time](https://github.com/kabass/commcare-hq-benin/blob/56682492f20c60cdef0ccde6049b9945b3658493/corehq/apps/es/REINDEX_PROCESS.md#reindexing-one-index-at-a-time)
...
5. The following steps should be performed serially for each canonical index name, replacing `${INDEX_CNAME}` with each of the following values `['apps', 'cases', 'case_search', 'domains', 'forms', 'groups', 'sms', 'users']`
    ...
    3. Verify the reindex is completed by querying the logs and ensuring that the doc count matches between primary and secondary indices. The commands in this step should be run from the control machine.
        ...
        3. Then verify doc counts match between primary and secondary indices using:
            ...
            If reindex fails for any index, please refer to the docs [here](https://github.com/kabass/commcare-hq-benin/blob/56682492f20c60cdef0ccde6049b9945b3658493/corehq/apps/es/REINDEX_PROCESS.md#common-issues-resolutions-during-reindex) for troubleshooting steps.
